...
class Album:
    ...
    def get_uniq_author(self):
        uniq_author = set()
        # return [uniq_author.add(song.author) for song in self.music]
        for song in self.music:
...
class MusicLibrary:
    ...
    def add_album(self, album):
        self.sound.append(album)

    def get_author_album(self, author):
        author_songs = set()
        for album in self.sound:
    ...
    def get_uniq_music_album(self):
        # music_set = [set(album.music()) for album in self.sound] НЕ РАБОТАЕТ
        music_set = []
        for album in self.sound:
            y = set(album.music)
            music_set.append(y)
...
album_1 = Album("Fovorits", "Disco")
album_2 = Album("Party", "Violet")
album_3 = Album("The road", "Melon")
...
# я правда не понимаю почему выводиться ссылка на экземпляр класса
